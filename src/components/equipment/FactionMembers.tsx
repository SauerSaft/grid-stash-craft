  ...
  return (
    <div className="ginshi_members_container flex flex-col flex-1 gap-3 overflow-hidden">
      ...
      <div className="ginshi_members_table flex-1 rounded-sm border border-white/[0.06] bg-white/[0.02] overflow-hidden flex flex-col">
        {/* Table Header */}
        <div className="ginshi_members_thead grid grid-cols-[1fr_140px_80px_130px] gap-2 px-4 py-2.5 border-b border-white/[0.06] bg-white/[0.02]">
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Name</span>
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Rang</span>
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground text-center">Status</span>
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground text-right">Aktionen</span>
        </div>

        {/* Table Body */}
        <div className="ginshi_members_tbody flex-1 overflow-y-auto">
          {members
            .sort((a, b) => b.rankLevel - a.rankLevel || a.name.localeCompare(b.name))
            .map((member) => (
              <div
                key={member.id}
                className="ginshi_member_row grid grid-cols-[1fr_140px_80px_130px] gap-2 items-center px-4 py-2 border-b border-white/[0.03] last:border-b-0 hover:bg-white/[0.02] transition-colors"
              >
                {/* Name */}
                ...
                <div className="flex items-center gap-1.5 min-w-0">
                  <div className={`w-1 h-1 rounded-full flex-shrink-0 ${member.rankLevel >= 4 ? "bg-primary" : member.rankLevel >= 3 ? "bg-info" : "bg-muted-foreground/40"}`} />
                ...
                <div className="flex justify-center">
                  <div className={`flex items-center gap-1 px-2 py-0.5 rounded-sm text-[9px] font-bold uppercase tracking-wider ${
                    member.online
