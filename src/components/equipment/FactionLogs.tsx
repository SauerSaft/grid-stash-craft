...
import { ScrollText, ShoppingCart, Shield, Package, Landmark, ArrowDownToLine, ArrowUpFromLine, ChevronLeft, ChevronRight } from "lucide-react";
...
interface ShopLog extends BaseLog {
  weapon: string;
}

interface ArmoryLog extends BaseLog {
  weapon: string;
  direction: "in" | "out";
}

interface StorageLog extends BaseLog {
  item: string;
  amount: number;
  direction: "in" | "out";
}

interface TreasuryLog extends BaseLog {
  amount: number;
  type: "deposit" | "withdraw";
}

// Mock data generators
const generateShopLogs = (): ShopLog[] => {
  ...
  return Array.from({ length: 87 }, (_, i) => ({
    ...
    date: `0${Math.floor(Math.random() * 7) + 1}.03.26 ${String(Math.floor(Math.random() * 24)).padStart(2, "0")}:${String(Math.floor(Math.random() * 60)).padStart(2, "0")}`,
  }));
};

const generateArmoryLogs = (): ArmoryLog[] => {
  const names = ["Max Müller", "Leon Schmidt", "Anna Weber", "Lukas Fischer", "Sophie Becker"];
  const weapons = ["Karabiner", "Pistole", "SMG", "Schrotflinte"];
...
const generateStorageLogs = (): StorageLog[] => {
  const names = ["Max Müller", "Leon Schmidt", "Anna Weber", "Jonas Wagner", "Tim Braun"];
...
const generateTreasuryLogs = (): TreasuryLog[] => {
  const names = ["Max Müller", "Leon Schmidt", "Anna Weber", "Lukas Fischer"];
...
const shopLogs = generateShopLogs();
const armoryLogs = generateArmoryLogs();
const storageLogs = generateStorageLogs();
const treasuryLogs = generateTreasuryLogs();

const ITEMS_PER_PAGE = 15;

const tabs: { id: LogTab; label: string; icon: typeof ShoppingCart }[] = [
  { id: "waffenshop", label: "Waffenshop", icon: ShoppingCart },
  { id: "waffenkammer", label: "Waffenkammer", icon: Shield },
  { id: "fraklager", label: "Fraklager", icon: Package },
  { id: "frakkasse", label: "Frakkasse", icon: Landmark },
];
...
const FactionLogs = () => {
  ...
  const getLogsForTab = () => {
    switch (activeTab) {
      case "waffenshop": return shopLogs;
      case "waffenkammer": return armoryLogs;
      case "fraklager": return storageLogs;
      case "frakkasse": return treasuryLogs;
    }
  };

  const allLogs = getLogsForTab();
  ...
  const renderRow = (log: any) => {
    switch (activeTab) {
      case "waffenshop":
        return (
          <div key={log.id} className="ginshi_log_row grid grid-cols-[50px_1fr_1fr_100px] items-center px-3 py-[7px] border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors">
      ...
      case "waffenkammer":
        ...
        return (
          <div key={log.id} className="ginshi_log_row grid grid-cols-[50px_1fr_1fr_100px] items-center px-3 py-[7px] border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors">
            ...
            <div className="flex items-center gap-1.5">
              {isOut
                ? <ArrowUpFromLine size={10} className="text-destructive/80" />
                : <ArrowDownToLine size={10} className="text-success/80" />
              }
        ...
        return (
          <div key={log.id} className="ginshi_log_row grid grid-cols-[50px_1fr_1fr_100px] items-center px-3 py-[7px] border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className={`text-[9px] font-bold tabular-nums px-1.5 py-0.5 rounded-sm ${
                sOut
                  ? "bg-destructive/10 border border-destructive/20 text-destructive"
                  : "bg-success/10 border border-success/20 text-success"
              }`}>
  ...
  return (
    <div className="ginshi_logs_container flex flex-col flex-1 gap-3 overflow-hidden">
      {/* Page Header */}
      <div className="ginshi_logs_header flex items-center gap-3 px-4 py-3 bg-white/[0.03] rounded-sm border border-white/[0.06]">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-xs font-medium text-muted-foreground">Alle Aktivitäten der Fraktion im Überblick</span>
        </div>
        <div className="ml-auto flex items-center gap-1.5 px-2.5 py-1 rounded-sm bg-white/[0.03] border border-white/[0.06]">
          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Einträge</span>
          <span className="text-xs font-extrabold tabular-nums text-primary">{allLogs.length}</span>
        </div>
      </div>

      {/* Tab Bar */}
      <div className="ginshi_logs_tabs flex gap-1">
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-sm text-[11px] font-bold uppercase tracking-wider transition-all ${
                isActive
      ...
      {/* Table */}
      <div className="ginshi_logs_table flex-1 overflow-hidden rounded-sm border border-white/[0.06] bg-white/[0.02] flex flex-col">
        {/* Table Header */}
        <div className="grid grid-cols-[50px_1fr_1fr_100px] px-3 py-2 border-b border-white/[0.08] bg-black/50 sticky top-0 z-10">
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="ginshi_logs_pagination flex items-center justify-between px-1">
